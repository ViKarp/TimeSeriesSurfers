...
class BaseCoach(ABC):
    def __init__(self, data_processor, memory, trigger, model, train_data, target):
        """
    ...
    def train(self, train_data):
        ...
        eval_metric = self.model.evaluate(np.arange(self.last_index + 1, self.last_index + 1 + len(val_data_scaled)),
                                          val_data_scaled)
        print(f"Eval metrics:  Avg loss: {eval_metric:2f} \n")
    ...
    def generate_graphics(self, dir_path):
        pass
